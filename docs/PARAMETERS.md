...
(`docs/measurements/parameter_table.md`) — 65 parameters. "In presets" = stored in and
...
* **Factory library**: 46 presets in 12 categories — PADS, PLUCKED, STRUCK, BOWED, AIR,
  GLASS, METAL, WOOD, MEMBRANE, DRONES, PERCUSSION, EXPERIMENTAL — including every example
  name from the product specification. Defined in code (`Core/FactoryPresets.cpp`) with a
  builder that places nodes exactly on harmonic ratios (`radiusForRatio`: the inverse of
  the voice's tuning map, including material ratio, TENSION stretch and detune).
* **Audit** (`presets/every factory preset renders cleanly`): each preset renders a chord;
  all finite, unclipped (release candidate: worst peak −4.8 dBFS), no DC; momentary loudness
  −18.4 … −27.9 dB; the closest spectral pair (Resonant Fog / Ghost Membrane) is 2.0 dB
  apart (distinct sounds). Results: `docs/measurements/phase9/factory_presets.csv` (rewritten
  by every test run), `docs/measurements/release/measurements.csv`, WAVs in `Tests/output/presets/`.
